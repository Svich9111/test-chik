const API_URL = 'https://sable-low-story.glitch.me/';
...
const renderPrice = (wrapper, data) => {
    data.forEach((item) => {
        ...
        priceItem.innerHTML = `
            <span class="ptice__item-title">${item.name}</span>
    ...
    /*<li class="price__item">
        <span class="ptice__item-title">Оформление бровей</span>
...
const renderSpec = (wrapper, data) => {
    const labels = data.map(item => {
        ...
        <span class="radio__label radio__label_spec" style="--bg-image: url(${API_URL}${item.img})">${item.name}
        `;
        return label;
    });
